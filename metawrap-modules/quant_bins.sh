...
help_message () {
	echo ""
	echo "Usage: metaWRAP quant_bins [options] -b bins_folder -o output_dir -a assembly.fa readsA_1.fastq readsA_2.fastq ... [readsX_1.fastq readsX_2.fastq] or readsA.fq1.gz readsA.fq2.gz ... readsX.fq1.gz readsX.fq2.gz"
	echo "Options:"
	echo ""
...
# default params
threads=8; out=false; bin_folder=false; assembly=false
...
F="no"; R="no"
for num in "$@"; do
	if [[ $num == *"_1.fastq" || $num == *".fq1.gz" ]]; then F="yes"; fi
	if [[ $num == *"_2.fastq" || $num == *".fq2.gz" ]]; then R="yes"; fi
done
if [ $F = "no" ] || [ $R = "no" ]; then
...
num_of_F_read_files=$(for I in "$@"; do echo $I | egrep '_1.fastq|fq1.gz'; done | wc -l)
num_of_R_read_files=$(for I in "$@"; do echo $I | egrep '_2.fastq|fq2.gz'; done | wc -l)
...
if [[ -s ${out}/assembly_index/hash.bin ]]; then  
comm "Indexing files existed, pass to alignment"
else
salmon index -p $threads -t $assembly -i ${out}/assembly_index; fi
...
line=$(ls ${out}/quant_files/ | grep counts | wc -l)
if [[ $line -gt 1 ]]; then
comm "Alignment has been made, PASS" 
else
...
for arg in "$@"; do
	...
	elif [[ $arg == *".fq1.gz"* ]]; then
                reads_1=$arg
                reads_2=${arg%.fq*}.fq2.gz
                if [ "$reads_2" = "false" ]; then error "$reads_2 does not exist. Exiting..."; fi
